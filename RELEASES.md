# Release Procedure for action-post-clang-tidy-results
...
gh release create "${NEXT_VERSION}" \
  ...
  --generate-notes \
  --repo Framework-R-D/action-post-clang-tidy-results
...
gh release view "${NEXT_VERSION}" --repo Framework-R-D/action-post-clang-tidy-results
...
NEW_SHA=$(gh api repos/Framework-R-D/action-post-clang-tidy-results/git/ref/tags/${NEXT_VERSION} \
  --jq .object.sha)
# If the tag is annotated, dereference:
TYPE=$(gh api repos/Framework-R-D/action-post-clang-tidy-results/git/ref/tags/${NEXT_VERSION} \
  --jq .object.type)
if [ "$TYPE" = "tag" ]; then
  NEW_SHA=$(gh api repos/Framework-R-D/action-post-clang-tidy-results/git/tags/${NEW_SHA} \
    --jq .object.sha)
fi

# Update README.md
OLD_PATTERN="Framework-R-D/action-post-clang-tidy-results@[0-9a-f]\{40\} # v[0-9]*"
NEW_STRING="Framework-R-D/action-post-clang-tidy-results@${NEW_SHA} # ${NEXT_VERSION}"
